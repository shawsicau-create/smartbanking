...
\centering
\begin{tikzpicture}[
  phase/.style={rectangle, rounded corners=4pt, draw=blue!70, very thick, fill=blue!8, minimum width=2.6cm, minimum height=2.0cm, align=center, font=\footnotesize, inner sep=4pt},
  phasehot/.style={rectangle, rounded corners=4pt, draw=orange!80, very thick, fill=orange!15, minimum width=2.6cm, minimum height=2.0cm, align=center, font=\footnotesize, inner sep=4pt},
  arr/.style={-{Stealth[length=3mm]}, very thick, blue!60},
  roleg/.style={ellipse, draw=red!70, very thick, fill=yellow!20, minimum width=1.8cm, minimum height=0.9cm, align=center, font=\footnotesize\bfseries, inner sep=2pt},
]
% 五个阶段
\node[phase]    (p1) at (0,  2.4)  {\textbf{P1 分析/构思}\\\textcolor{blue!70!black}{产品愿景}};
\node[phase]    (p2) at (3.5,2.4)  {\textbf{P2 规划/设计}\\\textcolor{blue!70!black}{PRD + 架构}};
\node[phasehot] (p3) at (7.0,2.4)  {\textbf{P3 方案细化}\\\textcolor{orange!80!black}{史诗 / 故事拆分}};
\node[phasehot] (p4) at (10.5,2.4) {\textbf{P4 实施/交付}\\\textcolor{orange!80!black}{Sprint 迭代}};
\node[phasehot] (p5) at (14.0,2.4) {\textbf{P5 反馈/优化}\\\textcolor{orange!80!black}{追溯 + 迭代}};
...
% AI 角色关联（每个阶段下对应一个 AI 角色）
\node[roleg] (r1) at (0, 0)  {Analyst};
\node[roleg] (r2) at (3.5,0)  {PM + Architect};
\node[roleg] (r3) at (7.0,0)  {Scrum Master};
\node[roleg] (r4) at (10.5,0) {Dev Team};
\node[roleg] (r5) at (14.0,0) {QA Owner};

% 阶段与角色连线
\draw[arr, dashed, gray!70] (p1) -- (r1);
\draw[arr, dashed, gray!70] (p2) -- (r2);
\draw[arr, dashed, gray!70] (p3) -- (r3);
\draw[arr, dashed, gray!70] (p4) -- (r4);
\draw[arr, dashed, gray!70] (p5) -- (r5);

% 闭环反馈
\draw[arr, red!70, very thick, dashed] (p5.south) to[bend right=40] (p1.south);
\node[draw=red!70, fill=white, font=\footnotesize\bfseries, align=center] at (7.0, -2.0) {闭环反馈：每个 Sprint 都贡献反馈给下个 P1};

% 顶部人类监督
\node[draw=red!70, very thick, fill=yellow!15, rounded corners=2pt, font=\footnotesize\bfseries, align=center, minimum width=11cm] at (7.0, 4.4) {人类决策者（项目经理 / 架构师）负责决策与审核};
\draw[arr, red!70] (7.0, 4.0) -- (p2.north);
\end{tikzpicture}
\caption{BMAD 五阶段闭环与 AI 角色分工示意（自制）}
\label{fig:bmad-cycle}
\end{figure}
