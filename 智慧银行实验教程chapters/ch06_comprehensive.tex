...
\keyterms{BMAD方法论、AI驱动开发、头脑风暴、PRD、架构设计、史诗拆分、Sprint规划、CRM系统、智能客服、Vercel部署、CNB}
...
\subsection{实验十二：Vercel部署（前端）}

\textbf{目标}：将前端项目部署到Vercel平台，获取公开访问URL。

\subsubsection*{前置准备：注册Vercel账户}

访问\url{https://vercel.com/signup}，选择注册方式（GitHub / GitLab / Bitbucket / 邮箱），完成注册并选择Hobby（免费）计划。

\subsubsection*{安装Vercel CLI}
...
如果出现\texttt{EPERM}或\texttt{EACCES}错误，以管理员身份打开PowerShell后重试，或使用\texttt{npx vercel}替代全局安装。
\end{notebox}

\subsubsection*{登录Vercel账户}

\begin{lstlisting}[style=shell]
vercel login
\end{lstlisting}

选择登录方式（GitHub / GitLab / Bitbucket / Email），完成授权。执行\texttt{vercel whoami}验证登录状态。

\textbf{非交互式登录（CI/CD环境）}：在\url{https://vercel.com/account/tokens}创建Token，然后使用：
...
\textbf{信息项} & \textbf{说明} \\
\midrule
生产URL & \texttt{https://frontend-xxx.vercel.app} \\
构建方式 & Vite自动识别 \\
输出目录 & \texttt{dist/} \\
构建时间 & $\sim$16s（含依赖安装） \\
\bottomrule
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{银行CRM系统完整开发}：通过12个递进式实验，完整走通了从BMAD安装、PRD创建、架构设计、UX设计、Epic拆分、Sprint规划、代码实现、数据库适配、功能测试、CNB推送到Vercel部署的全流程，产出了一个具备客户管理、贷款申请、即申即贷等功能的完整CRM系统
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item CNB的OAuth登录和PAT令牌两种认证方式，各有什么优缺点？如果要将后端也部署到云端（而非仅前端部署Vercel），有哪些方案？
...
Vercel CLI 文档 & \url{https://vercel.com/docs/cli} \\
