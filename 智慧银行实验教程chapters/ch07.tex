...
\begin{enumerate}
  ...
  \item 了解因果推断的主要方法论（DID/RDD/IV/SCM/PSM）
  \item 掌握AI辅助计量分析的完整工作流
  \item 掌握金融数据获取 Skill（\texttt{tushare-finance} / \texttt{china-stock-analysis} / \texttt{openbb-finance} / \texttt{api-data-fetcher}）的调用
  \item 掌握计量分析 Skill（\texttt{stata} / \texttt{stata-regression} / \texttt{r-econometrics} / \texttt{python-panel-data} / \texttt{data-analysis}）的差异化使用与三环境协同
  \item 掌握发表级图表与LaTeX表格 Skill（\texttt{econ-visualization} / \texttt{paper-figure} / \texttt{latex-tables} / \texttt{mermaid-diagram}）的输出
  \item 掌握计量研究演示文稿 Skill（学术线 \texttt{paper-slides} / \texttt{beamer-presentation}；商业线 \texttt{pptx} / \texttt{qoderwork-ppt} / \texttt{ppt-master}）
\end{enumerate}
}

% ============================================================
\section{AI Skill辅助金融数据分析工作流}

% ============================================================

\textbf{【本章方法论定位】} 本章所有计量分析任务都以"调用Skill"为组织原则——不仅是“写一段提示词让AI跑代码”，更是明确调用\texttt{stata} / \texttt{r-econometrics} / \texttt{python-panel-data} / \texttt{econ-visualization}等Skill来完成专业金融计量任务。

\textbf{与ch03的MCP流水线区别}：本章Skill是"方法论"（怎么微计量结果处理），MCP是"工具能力"（能不能读到数据）。二者是能力与方法的辩证统一。

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  node distance=1.6cm,
  step/.style={rectangle, rounded corners=4pt, draw=blue!70, very thick, fill=blue!8, minimum width=2.6cm, minimum height=1.6cm, align=center, font=\footnotesize, inner sep=4pt},
  stephot/.style={rectangle, rounded corners=4pt, draw=orange!80, very thick, fill=orange!15, minimum width=2.6cm, minimum height=1.6cm, align=center, font=\footnotesize, inner sep=4pt},
  arr/.style={-{Stealth[length=3mm]}, very thick, blue!60},
]
% 6 个节点，水平铺开
\node[step]    (n1) at (0, 0)  {\textbf{1. 数据获取}\\\textcolor{blue!70!black}{api-data-fetcher}\\\textcolor{blue!70!black}{cn-stock-mcp}\\\textcolor{blue!70!black}{tushareMcp}};
\node[step]    (n2) at (3.4,0)  {\textbf{2. 数据清洗}\\\textcolor{blue!70!black}{stata-data-cleaning}\\\textcolor{blue!70!black}{python-panel-data}};
\node[stephot] (n3) at (6.8,0)  {\textbf{3. 计量回归}\\\textcolor{orange!80!black}{stata-regression}\\\textcolor{orange!80!black}{r-econometrics}};
\node[stephot] (n4) at (10.2,0) {\textbf{4. 因果推断}\\\textcolor{orange!80!black}{abm-toolkit}\\\textcolor{orange!80!black}{python-panel-data}};
\node[step]    (n5) at (13.6,0) {\textbf{5. 时间序列}\\\textcolor{blue!70!black}{shibor / LPR 数据}\\\textcolor{blue!70!black}{forecast Skill}};
\node[stephot] (n6) at (17.0,0) {\textbf{6. 表格与图表}\\\textcolor{orange!80!black}{latex-tables}\\\textcolor{orange!80!black}{econ-visualization}\\\textcolor{orange!80!black}{paper-figure}};
...
% 阶段背景标
\node[draw=blue!40, dashed, thick, rounded corners=2pt, fit=(n1)(n2), inner sep=6pt, label=above:\footnotesize 预处理] (stage1) {};
\node[draw=orange!60, dashed, thick, rounded corners=2pt, fit=(n3)(n4), inner sep=6pt, label=above:\footnotesize 核心分析] (stage2) {};
\node[draw=blue!40, dashed, thick, rounded corners=2pt, fit=(n5)(n6), inner sep=6pt, label=above:\footnotesize 输出] (stage3) {};

% 反馈环
\draw[arr, red!70, dashed] (n6.south) to[bend right=30] (n1.south);
\node[font=\footnotesize\bfseries, red!70] at (8.5, -2.4) {迭代：调参 / 补数据 / 稳健性检验};
\end{tikzpicture}
\caption{AI Skill 辅助金融数据分析 6 节点流水线（自制）}
\label{fig:data-pipeline}
\end{figure}

\subsection{6节点Skill流水线（贯穿本章）}

\begin{table}[H]
\centering
\begin{tabular}{clp{6cm}p{4.5cm}}
\toprule
\textbf{节点} & \textbf{环节} & \textbf{任务} & \textbf{Skill候选} \\
\midrule
1 & 文献综述 & 系统检索+综述+管理引用 & \texttt{systematic-literature-review} / \texttt{lit-review-assistant} / \texttt{academic-researcher} / \texttt{find-literature} \\
2 & 数据获取 & 从API/数据库拉取金融数据 & \texttt{tushare-finance} / \texttt{china-stock-analysis} / \texttt{api-data-fetcher} / \texttt{worldbank-data360} / \texttt{oecd-data} \\
3 & 数据清洗 & 缺失值/异常值/变换 & \texttt{stata-data-cleaning} / \texttt{data-analysis} / \texttt{r-data-cleaning} \\
4 & 回归分析 & OLS/FE/IV/DiD/RDD/预测 & \texttt{stata} / \texttt{stata-regression} / \texttt{r-econometrics} / \texttt{python-panel-data} / \texttt{data-analysis} \\
5 & 图表生成 & 发表级图表与表格 & \texttt{econ-visualization} / \texttt{paper-figure} / \texttt{paper-illustration} / \texttt{latex-tables} / \texttt{mermaid-diagram} \\
6 & 论文与演示 & LaTeX初稿与会议slides & \texttt{paper-write} / \texttt{academic-paper-writer} / \texttt{paper-slides} / \texttt{beamer-presentation} / \texttt{ppt-master} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{提示词Skill化改写原则}

为实现“所有Stata/Python/R提示词都Skill化”，本章遵循统一的提示词改写原则：

\textbf{三要素包装法}（每个原提示词都必做）：

原始提示词 = "请运行Stata代码：reg y x, robust"

Skill化后 = "请调用 \texttt{stata} Skill完成以下任务：【背景】...【输入】...【代码】...【预期输出】..."

\textbf{改写原则10条}：
\begin{enumerate}[leftmargin=2em]
  \item 必须明示Skill名称（调用X Skill）
  \item 必须明示输入文件/变量
  \item 必须明示输出文件/格式
  \item 原裸代码保留（作为代码补充，不是替代）
  \item 环境检查提前（提示词开头或附录说明依赖）
  \item 多Skill串联（一个提示词可调用多个Skill协同）
  \item 失败回退方案（如Skill不可用，如何手动运行）
  \item 输出预期明确（交什么文件、什么格式）
  \item 业务上下文（为什么跑这个回归，解释预期含义）
  \item 可重复性（明确随机种子、软件版本、依赖版本）
\end{enumerate}

\textbf{标准化提示词模板}（实验报告附件用）：
\begin{lstlisting}
【Skill调用提示词 v1.0】
...
% ============================================================
\section{计量分析运行环境准备（R/Stata/Python三环境协同）}

% ============================================================

本章计量分析依赖R、Stata、Python三大工具中任一环境，本节在调用计量Skill之前提供完整环境准备说明。

\subsection{R环境（r-econometrics Skill前置）}
...
\textbf{RStudio Desktop}（推荐IDE）：\url{https://posit.co/download/rstudio-desktop/}

\textbf{R路径验证脚本}（运行后可粘贴到AI提示词）：
\begin{lstlisting}[style=r]
...
\subsection{Stata环境（stata / stata-regression Skill前置）}
...
\textbf{Stata与R/Python互调}（\texttt{stata} Skill支持）：
\begin{lstlisting}[style=stata]
python: import pandas as pd; df = pd.read_stata("data.dta")
rsource: library(fixest); source("fe_reg.R")
\end{lstlisting}

\subsection{Python环境（python-panel-data / data-analysis Skill前置）}

\textbf{建议版本}：Python 3.10+。
...
\subsection{三环境协同使用提示}

\begin{table}[H]
\centering
\begin{tabular}{|l|p{4cm}|p{7cm}|}
\hline
\textbf{工具} & \textbf{优势} & \textbf{本章典型场景} \\
\hline
R & 计量生态最完整（fixest/did/AER），可重复性高 & \texttt{r-econometrics} Skill处理DiD/RDD/IV/合成控制 \\
Stata & 中文金融实证主流（reghdfe/eventdd），可发表性高 & \texttt{stata-regression} Skill输出三列回归表 \\
Python & 机器学习/深度学习强，灵活性高 & \texttt{python-panel-data} Skill做LSTM/Prophet预测 \\
\hline
\end{tabular}
\end{table}

% ============================================================
\section{全章Skill环境检查清单（期末上机前必运行）}

% ============================================================

本节是ch07所有用到的Skill环境检查总表，覆盖约30个Skill，每个Skill列出安装指令、环境检查命令、在ch07中出现的位置。

\subsection{数据获取Skill（5个）}
\textbf{tushare-finance Skill}（任务13、18）：
\begin{lstlisting}[style=shell]
pip install tushare pandas
export TUSHARE_TOKEN='your_token_here'
python3 -c "import tushare; pro=tushare.pro_api(); print(pro.stock_basic(list_status='L', limit=1))"
\end{lstlisting}
\textbf{api-data-fetcher Skill}（任务13）：\texttt{pip install requests pandas}。

\textbf{worldbank-data360 Skill}（任务17）：联网即可。

\textbf{oecd-data Skill}（任务17）：联网即可。

\textbf{china-stock-analysis Skill}（任务18）：依赖china-stock-mcp MCP，需mcp.json配置。

\subsection{数据清洗Skill（3个）}
\begin{itemize}[leftmargin=1em]
  \item \textbf{stata-data-cleaning Skill}（任务13）：见§Stata环境
  \item \textbf{data-analysis Skill}（任务13、15、18）：见§Python环境
  \item \textbf{r-data-cleaning Skill}（任务13）：依赖\texttt{dplyr}/\texttt{tidyr}（tidyverse的一部分）
\end{itemize}

\subsection{计量回归Skill（5个）}
\begin{itemize}[leftmargin=1em]
  \item \textbf{stata Skill}（任务13-18）：见§Stata环境
  \item \textbf{stata-regression Skill}（任务14、18）：stata依赖 + reghdfe/estout
  \item \textbf{r-econometrics Skill}（任务14、16、18）：见§R环境
  \item \textbf{python-panel-data Skill}（任务14、15、18）：见§Python环境 + linearmodels
  \item \textbf{data-analysis Skill}（任务14-18）：见§Python环境
\end{itemize}

\subsection{因果推断Skill（5个）}
\textbf{r-econometrics Skill}：同上。

\textbf{did / csdid R包}（任务16）：
\begin{lstlisting}[style=shell]
Rscript -e 'library(did); packageVersion("did")'
\end{lstlisting}
\textbf{rdrobust / Synth / MatchIt R包}（任务16）：见§R环境。

\subsection{时间序列Skill（4个）}
\textbf{r-econometrics Skill}（任务15）：含\texttt{vars}/\texttt{urca}/\texttt{forecast}。

\textbf{statsmodels Python包}（任务15）：见§Python环境。

\textbf{pmdarima Python包}（任务15）：
\begin{lstlisting}[style=shell]
pip install pmdarima
python3 -c "import pmdarima; print(pmdarima.__version__)"
\end{lstlisting}
\textbf{prophet Python包}（任务15）：
\begin{lstlisting}[style=shell]
pip install prophet
python3 -c "from prophet import Prophet; print('prophet OK')"
\end{lstlisting}

\subsection{表格与图表Skill（5个）}
\begin{itemize}[leftmargin=1em]
  \item \textbf{latex-tables Skill}（任务14、18）：依赖\texttt{esttab}或\texttt{modelsummary}
  \item \textbf{econ-visualization Skill}（任务14-18）：见§R环境
  \item \textbf{paper-figure Skill}（任务15、18）：见§Python环境
  \item \textbf{paper-illustration Skill}（任务17、18）：需\texttt{GEMINI\_API\_KEY}
  \item \textbf{mermaid-diagram Skill}（任务17、18）：\texttt{npm install -g @mermaid-js/mermaid-cli}
\end{itemize}

\subsection{写作与文献Skill（4个）}
\begin{itemize}[leftmargin=1em]
  \item \textbf{paper-write / academic-paper-writer Skill}（任务17、18）：依赖xelatex
  \item \textbf{systematic-literature-review Skill}（任务17）：联网即可
  \item \textbf{lit-review-assistant Skill}（任务17）：联网即可
\end{itemize}

\subsection{ch07全章一键环境检查脚本（期末上机前必运行）}
...
\subsubsection{Skill 化包装：本节实操提示词}

\textbf{【本节 Skill 化原则】} 下面三个提示词覆盖了金融数据预处理的完整 Skill 化流水线，所有提示词都明确调用具体 Skill，原裸代码作为补充保留。
...
\textbf{Skill 调用推荐}：Tushare 适合批量拉取学术面板数据；china-stock-analysis MCP 适合实时数据查询；CSMAR/Wind 数据需要商业订阅。
...
\textbf{【本节 Skill 化原则】} 本节所有 Stata/Python/R 提示词都包装为 Skill 调用形式，下方提供 4 个核心场景的 Skill 化提示词，覆盖原裸代码 + 三环境对比。

\textbf{三模型面板回归（Stata Skill 化）}：
\begin{lstlisting}
...
\textbf{R fixest 多维 FE（R Skill 化）}：
\begin{lstlisting}
...
\textbf{回归系数对比可视化（econ-visualization Skill 化）}：
\begin{lstlisting}
...
\textbf{三环境 Skill 对比}：上述四个提示词覆盖了 Stata（中文金融主流）、R（计量方法论完整）、Python（机器学习兼容）三环境，学生可根据自己熟悉的工具选择其一，或交叉验证。
...
\textbf{【本节 Skill 化原则】} 本节所有 SARIMA/Prophet/LSTM/GRU 代码都包装为 Skill 调用形式，下方提供 5 个核心场景的 Skill 化提示词。

\textbf{STL 时间序列分解（Python Skill 化）}：
\begin{lstlisting}
...
\textbf{SARIMA 自动定阶（Python Skill 化）}：
\begin{lstlisting}
...
\textbf{LSTM 一键构建（Python Skill 化）}：
\begin{lstlisting}
...
\textbf{Prophet 快速预测（Python Skill 化）}：
\begin{lstlisting}
...
\textbf{模型对比可视化（paper-figure Skill 化）}：
\begin{lstlisting}
...
\textbf{【本节 Skill 化原则】} 本节所有 DID/RDD/IV/SCM/PSM 代码都包装为 Skill 调用形式，下方提供 6 个核心场景的 Skill 化提示词。

\textbf{DID 双重差分（Stata + R 双 Skill 化）}：
\begin{lstlisting}
...
\textbf{IV 工具变量（R + Python 双 Skill 化）}：
\begin{lstlisting}
...
\textbf{PSM 倾向得分匹配（R + Stata 双 Skill 化）}：
\begin{lstlisting}
...
\textbf{文献梳理前置（systematic-literature-review Skill 化）}：
\begin{lstlisting}
...
\subsubsection{Step 1：文献综述→确定模型设定（Skill 化版）}

利用 \texttt{systematic-literature-review} / \texttt{lit-review-assistant} / \texttt{academic-researcher} 元 Skill 快速梳理文献：
...
\subsubsection{Step 2：数据获取与清洗（Skill 化版）}

调用 \texttt{tushare-finance} / \texttt{worldbank-data360} / \texttt{oecd-mcp} 获取数据，\texttt{data-analysis} / \texttt{r-data-cleaning} 完成清洗：
...
\subsubsection{Step 3：描述统计与可视化（Skill 化版）}

调用 \texttt{data-analysis} / \texttt{econ-visualization} 完成描述统计：
...
\subsubsection{Step 4：模型估计与检验（Skill 化版）}

调用 \texttt{stata-regression} / \texttt{python-panel-data} / \texttt{r-econometrics} 三选一执行模型估计：
...
\subsubsection{Step 5：稳健性检验设计（Skill 化版）}

调用 \texttt{econ-visualization} 完成稳健性图形展示：
...
\subsubsection{Step 6：结果解释与论文撰写（Skill 化版）}

调用 \texttt{paper-write} / \texttt{academic-paper-writer} / \texttt{latex-tables} 输出 LaTeX 初稿：
...
\begin{warningbox}[AI 计量分析的常见陷阱 + Skill 误调用风险]
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Skill 误调用风险（新增）}：
    \begin{itemize}
      \item \textbf{Skill 重复触发}：多个 Skill 提供相似功能（如 python-panel-data 与 data-analysis），AI 可能误调用多个 Skill 导致输出冲突。建议在提示词中明确指定单一主 Skill。
      \item \textbf{Skill 版本不一致}：相同 Skill 不同版本的输出格式可能不同（如 latex-tables v1 输出三线表，v2 输出 booktabs），建议锁定版本号。
      \item \textbf{Skill 数据要求差异}：stata-regression 接受 .dta，python-panel-data 接受 .csv，r-econometrics 接受 .csv/RData。切换 Skill 时需检查数据格式。
      \item \textbf{Skill 误触发其他章节}：如 §6.1 数据特征 调用 data-analysis Skill 后，AI 误判该 Skill 应继续处理 §6.2 面板回归，导致数据未清洗即进入回归阶段。
    \end{itemize}
\end{enumerate}
\end{warningbox}

% ============================================================
\section{计量分析成果可视化Skill（独立章节）}

% ============================================================

\textbf{【本章特别要求】} 本节专门介绍计量分析成果的可视化Skill体系，涵盖学术图表、LaTeX表格、Mermaid示意图。

\subsection{econ-visualization（学术发表级图表）}
\textbf{适用场景}：所有计量结果的图形化输出。
\begin{itemize}[leftmargin=1em]
  \item \textbf{描述统计}：分布直方图、相关矩阵热力图
  \item \textbf{面板回归}：系数对比图、固定效应分布图
  \item \textbf{时间序列}：预测对比图、残差诊断图
  \item \textbf{因果推断}：\textbf{事件研究图}（含95\%置信区间）、\textbf{平行趋势检验图}、\textbf{断点回归图}、\textbf{倾向得分分布图}
\end{itemize}
\textbf{输出}：\texttt{figures/*.pdf}（矢量）。
...
\subsection{paper-figure（Python matplotlib论文图表）}
与\texttt{econ-visualization}互补：R ggplot2 vs Python matplotlib。

9类图表决策树自动选型（line/bar/scatter/heatmap/box/multi-panel）。与\texttt{paper-write} Skill联动：自动生成 \texttt{\textbackslash includegraphics} LaTeX片段。

\textbf{适用场景}：需要Python生态深度集成时（如scikit-learn模型可视化）。

\subsection{mermaid-diagram（流程图与示意图）}
\textbf{适用场景}：识别策略示意图、变量关系图、研究框架图。
...
\subsection{latex-tables（发表级LaTeX表格）}
\textbf{触发词}："回归表LaTeX"、"esttab"、"LaTeX表格"。

\textbf{能力清单}：
\begin{itemize}[leftmargin=1em]
  \item Stata \texttt{esttab}一键输出三线表
  \item R \texttt{modelsummary::modelsummary(list, output="table.tex")}
  \item Python \texttt{stargazer}或自定义booktabs风格
\end{itemize}
...
\subsection{计量可视化Skill选择决策树}

\begin{table}[H]
\centering
\begin{tabular}{|l|l|}
\hline
\textbf{需要什么？} & \textbf{推荐Skill} \\
\hline
发表级图表（黑白打印友好） & \texttt{econ-visualization} \\
需要Python生态集成 & \texttt{paper-figure} \\
识别策略流程图 / 框架图 & \texttt{mermaid-diagram} \\
三线表 / 回归表 & \texttt{latex-tables} \\
架构图 / 模型图 & \texttt{paper-illustration}（需GEMINI\_API\_KEY） \\
\hline
\end{tabular}
\end{table}

% ============================================================
\section{计量研究演示与汇报Skill（独立章节）}

% ============================================================

\textbf{【本章特别要求】} 本节专门介绍计量研究从图表到演示文稿的全套Skill体系，分学术与商业两条主线。

\subsection{学术汇报线（开题答辩/学术会议）}

\textbf{paper-slides Skill}（首选）：
\begin{itemize}[leftmargin=1em]
  \item \textbf{适用}：将完整的计量研究论文转为会议汇报slides
  \item \textbf{输入}：\texttt{paper/sections/*.tex} + \texttt{figures/}
  \item \textbf{输出}：\texttt{slides/main.pdf}（Beamer）+ \texttt{presentation.pptx} + \texttt{TALK\_SCRIPT.md}
  \item \textbf{4档规模}：poster-talk（5-8页）/ spotlight（8-12页）/ oral（15-22页）/ invited（25-40页）
  \item \textbf{7套会议配色}：NeurIPS / ICML / ICLR / CVPR / ECCV / AAAI / ACL / EMNLP / GENERIC
  \item \textbf{完整讲稿} + 预期Q\&A 8个
\end{itemize}
...
\textbf{beamer-presentation Skill}（手工定制Beamer）：
\begin{itemize}[leftmargin=1em]
  \item \textbf{适用}：开题答辩、毕业答辩、job market talk
  \item 提供metropolis/Madrid/CambridgeUS等主题
  \item 直接编辑.tex源文件，灵活度高
\end{itemize}

\textbf{paper-poster Skill}：
\begin{itemize}[leftmargin=1em]
  \item \textbf{适用}：学术会议海报展示（A0/A1幅面）
  \item 特点：单页视觉优先，包含QR码、图表嵌入
\end{itemize}

\subsection{商业/银行汇报线（董事会/客户汇报）}

\textbf{pptx Skill}（通用PPT操作）：
\begin{itemize}[leftmargin=1em]
  \item \textbf{适用}：行内决策汇报、客户提案
  \item 9种专业配色：Deep Night、Woodland、Vibrant Coral、Earth Tone等
  \item 视觉设计原则：避免默认蓝、加0.5" 留白、字号对比 ≥20pt
\end{itemize}

\textbf{qoderwork-ppt Skill}（14模板智能匹配）：
\begin{itemize}[leftmargin=1em]
  \item \textbf{适用}：商业汇报、产品发布、培训材料
  \item 14种HTML模板 + 一键管线（Puppeteer → dom-to-pptx）
  \item 环境：Node.js + Puppeteer + LibreOffice
\end{itemize}

\textbf{ppt-master Skill}（AI多格式 → SVG → PPTX）—— \textbf{【用户特别要求】}：
\begin{itemize}[leftmargin=1em]
  \item \textbf{适用}：现成调研报告 / PDF / Word / Markdown一键转专业PPT
  \item 多Agent协作：内容解析 → 视觉设计 → SVG渲染
  \item 输出：高质量SVG页面 + 可编辑PPTX
\end{itemize}

\textbf{与其他PPT Skill的差异}：
\begin{itemize}[leftmargin=1em]
  \item \texttt{paper-slides}：论文 → 学术Beamer slides
  \item \texttt{pptx}：通用PPT自由创作
  \item \texttt{qoderwork-ppt}：HTML模板匹配
  \item \textbf{\texttt{ppt-master}：现成文档智能转PPT（最适合作业/调研报告汇报）}
\end{itemize}

\subsection{演示文稿Skill选择决策树}

\begin{table}[H]
\centering
\begin{tabular}{|l|l|}
\hline
\textbf{需求场景} & \textbf{推荐Skill} \\
\hline
论文转会议汇报slides & \texttt{paper-slides} \\
开题/毕业答辩Beamer & \texttt{beamer-presentation} \\
学术海报 & \texttt{paper-poster} \\
行内决策汇报（手工定制） & \texttt{pptx} \\
商业产品发布（模板化） & \texttt{qoderwork-ppt} \\
调研报告/作业转PPT（最快速） & \texttt{ppt-master} \\
\hline
\end{tabular}
\end{table}

\subsection{计量研究端到端Skill流水线（贯穿全章）}
...
\begin{tabular}{lp{4cm}p{7cm}}
\toprule
\textbf{场景} & \textbf{原提示词} & \textbf{Skill化后提示词} \\
\midrule
Stata回归 & "跑reghdfe命令" & "请调用 \texttt{stata-regression} Skill跑reghdfe，使用 y、i.firm、c.x1\#\#c.x2、fe(year)，vce(cluster firm)，输出三列回归表到 ch07\_table1.tex" \\
Python LSTM & "训练LSTM预测ROA" & "请调用 \texttt{python-panel-data} Skill完成LSTM预测，输入数据 bank\_panel.csv，预测 horizon=5，输出 predictions.csv + paper-figure配套图表" \\
R断点回归 & "跑rdrobust" & "请调用 \texttt{r-econometrics} Skill实现断点回归：因变量 y、运行变量 x、断点 cutoff=0，输出 rd\_estimate.txt + econ-visualization生成断点图 rd\_plot.pdf" \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{【Skill化扩展】中文金融数据源对照表}：除 Bankscope 外，本实验可使用以下中文数据源（按 Skill 集成度排序）：

\begin{table}[H]
\centering
\begin{tabular}{lp{4cm}p{7cm}}
\toprule
\textbf{数据源} & \textbf{Skill / 工具} & \textbf{覆盖范围与特点} \\
\midrule
Tushare Pro & \texttt{tushare-finance} Skill（推荐） & 沪深A股、港股、基金、期货、宏观经济；免费 Token 即可访问；2010-至今 \\
Wind 资讯 & \texttt{hexin-ifind-ds-stock-mcp} & 银行财务、宏观、行业指数全；机构版订阅；高频与日频均覆盖 \\
CSMAR & \texttt{api-data-fetcher} + 自定义脚本 & 学术标准数据库；银行财务、公司治理、宏观研究系列 \\
同花顺 iFinD & \texttt{hexin-ifind-ds-stock-mcp} & 同 Wind，含研报、公告、新闻舆情 \\
银行年报 & \texttt{pdf-extraction} Skill & 银保监会披露的官方文本；最权威但需 OCR \\
宏观数据 & \texttt{worldbank-data360} / \texttt{oecd-mcp} & 跨国宏观对照；GDP、CPI、M2 等 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{建议策略}：本科教学用 \texttt{tushare-finance} Skill（免费）；硕士论文用 CSMAR/Wind；跨国比较用 World Bank/OECD。
...
\subsection{Skill 化包装概述}

\textbf{【本实验 Skill 化原则】} 本实验的所有 Stata/Python/R 提示词都包装为 Skill 调用形式，每个 Step 提供双版本提示词：\textbf{① Skill 化包装提示词}（推荐使用）+ \textbf{② 原始裸代码提示词}（作为代码补充保留）。

\textbf{统一提示词模板}（每个 Step 都遵循）：
...
\textbf{交付物格式升级}：本实验所有 .rtf/.txt 输出\textbf{同时输出 .tex}（由 \texttt{latex-tables} Skill 接管）+ \textbf{PDF}（由 \texttt{paper-compile} Skill 编译），方便直接嵌入论文。

\textbf{Skill 调用记录文件}：实验结束后必须生成 \texttt{reports/ch07\_skill\_workflow.md}（完整 Skill 调用日志），包含：每次调用的 Skill 名称、输入输出文件、执行时长、错误信息与回退方案。该文件作为实验报告附件提交，是 Skill 化实验的核心凭证。

\subsection{分步提示词}

\subsubsection{Step 1：描述统计与相关性分析}

\textbf{【Skill 化版】调用 \texttt{data-analysis} + \texttt{econ-visualization}}：
...
\textbf{【Skill 化版】调用 \texttt{stata-regression} + \texttt{latex-tables}}：
...
\textbf{【Skill 化版】调用 \texttt{stata-regression} + \texttt{econ-visualization} + \texttt{paper-figure}}：
...
\textbf{【Skill 化版】调用 \texttt{r-econometrics} 中介模块}：
...
\textbf{【Skill 化交付物清单】} 所有交付物同时输出 LaTeX + PDF 格式（除原始 .rtf/.txt 外）：

\begin{table}[H]
\centering
\begin{tabular}{clll}
\toprule
\textbf{\#} & \textbf{交付物} & \textbf{原格式} & \textbf{Skill 化输出格式} \\
\midrule
...
\textbf{7} & \textbf{【新增·核心交付物】Skill 调用记录} & --- & \texttt{reports/ch07\_skill\_workflow.md} \\
\textbf{8} & \textbf{【新增】配套图表集} & --- & \texttt{figures/ch07\_*.pdf}（5-8 张） \\
\textbf{9} & \textbf{【新增】Skill 化实验报告模板} & --- & \texttt{reports/ch07\_report.tex} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{\texttt{ch07\_skill\_workflow.md} 模板}：
...
\subsection{Skill 化交付物补充说明}

\textbf{为什么需要 \texttt{ch07\_skill\_workflow.md}}：
\begin{itemize}[leftmargin=2em]
  \item \textbf{可追溯性}：教师可通过该日志复现学生每次 Skill 调用，验证实验真实性。
  \item \textbf{调试凭证}：当输出文件缺失或错误时，可通过日志快速定位是哪个 Skill / 步骤出错。
  \item \textbf{Skill 习得证据}：展示学生从“裸命令”到“Skill 化工作流”的思维转变，是 Skill 化教学的核心产出。
  \item \textbf{可复用资产}：后续学生可参考本日志快速搭建同类实验。
\end{itemize}

\textbf{交付物文件清单（期末提交检查表）}：

\begin{table}[H]
\centering
\begin{tabular}{cll}
\toprule
\textbf{\checkmark} & \textbf{文件名} & \textbf{备注} \\
\midrule
\square & \texttt{data/bank\_panel\_clean.csv} & 数据准备 \\
\square & \texttt{reports/ch07\_desc\_stat.tex/.pdf} & 描述统计 \\
\square & \texttt{reports/ch07\_baseline.tex/.pdf} & 基准回归 \\
\square & \texttt{reports/ch07\_robustness.tex/.pdf} & 稳健性 \\
\square & \texttt{reports/ch07\_heterogeneity.tex/.pdf} & 异质性 \\
\square & \texttt{reports/ch07\_mediation.tex/.pdf} & 中介效应 \\
\square & \texttt{figures/ch07\_*.pdf} & 5-8 张配套图 \\
\square & \texttt{reports/ch07\_skill\_workflow.md} & \textbf{Skill 调用日志} \\
\square & \texttt{reports/ch07\_report.tex/.pdf} & 完整实验报告 \\
\square & \texttt{references/literature.bib} & 文献 BibTeX \\
\bottomrule
\end{tabular}
\end{table}
...
\keyterms{面板数据、双向固定效应模型、随机效应模型、Hausman检验、F检验、BP-LM检验、reghdfe、动态面板GMM、Arellano-Bond、SARIMA、LSTM、GRU、Prophet、VAR、VECM、单位根检验、协整、Granger因果、因果推断、双重差分法（DID）、平行趋势检验、Callaway-Sant'Anna、断点回归（RDD）、工具变量法（IV）、2SLS、合成控制法（SCM）、倾向得分匹配（PSM）、缩尾处理（Winsorize）、ARCH效应、GARCH、多重插补、中介效应、Sobel检验、Baron-Kenny、Skill化提示词、\texttt{stata-regression} Skill、\texttt{r-econometrics} Skill、\texttt{python-panel-data} Skill、\texttt{econ-visualization} Skill、\texttt{tushare-finance} Skill、\texttt{latex-tables} Skill、\texttt{stata-data-cleaning} Skill、\texttt{data-analysis} Skill、Skill调用日志（\texttt{ch07\_skill\_workflow.md}）}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{【Skill 化·新增】} 在面板回归分析中，\texttt{stata-regression} / \texttt{r-econometrics} / \texttt{python-panel-data} 三个 Skill 各有什么独特优势与使用场景？请设计一个“Stata 估计 + R fixest 交叉验证 + Python linearmodels GMM 对比”的三 Skill 协同工作流，并讨论如何处理三者在数据格式、输出风格、稳健性指标上的差异。
  \item \textbf{【Skill 化·新增】} \texttt{econ-visualization} Skill 能够自动生成发表级图表。请设计 3 个对比实验：\textbf{(a)} 未调用该 Skill 仅靠 AI 自行生成的图表 vs \textbf{(b)} 调用该 Skill 生成的图表 vs \textbf{(c)} 调用 \texttt{paper-figure} Skill 生成的图表，从期刊审稿人视角评价三种图表在“主题一致性、色彩友好性、矢量输出、可重复性”上的差异。
  \item \textbf{【Skill 化·新增】} 本章提供了 Tushare / iFinD / China-Stock / World Bank / OECD 等多个数据获取 Skill。请以“研究中国上市银行 2010-2024 年跨国比较”这一课题为例，设计一个多数据源融合策略：哪些指标优先用哪个 Skill？跨国宏观变量应使用哪个 Skill？如遇到 Skill 返回数据冲突，如何进行交叉验证与质量控制？
\end{enumerate}
\end{exercisebox}
