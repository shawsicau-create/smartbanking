...
\begin{enumerate}
  ...
  \item 掌握金融数据组MCP（Tushare / 同花顺iFinD / Yahoo Finance）在投研分析中的应用
  \item 掌握经济数据组MCP（OECD / World Bank / OpenEcon）在宏观研究中的应用
  \item 掌握Office组MCP在金融文档自动化中的应用
  \item 掌握图表可视化MCP（AntV Chart）在金融图表生成中的应用
  \item 掌握Stata MCP在计量经济学分析中的应用
  \item 了解地图、PDF文献、系统自动化、AI辅助等辅助MCP的使用
...
\centering
\begin{tikzpicture}[
  layer/.style={rectangle, rounded corners=4pt, draw=blue!70, very thick, fill=blue!8, minimum width=11cm, minimum height=1.4cm, align=center, font=\small, inner sep=4pt},
  layerhot/.style={rectangle, rounded corners=4pt, draw=orange!80, very thick, fill=orange!15, minimum width=11cm, minimum height=1.4cm, align=center, font=\small, inner sep=4pt},
  arrow/.style={-{Stealth[length=3mm]}, very thick},
  bidir/.style={{Stealth[length=3mm]}-{Stealth[length=3mm]}, very thick, dashed, gray!70},
]
% 三层架构（自上而下）
\node[layer] (L3) at (0, 0) {\textbf{第3层\quad MCP Client（宿主 IDE）}\quad TRAE / Qoder / Cursor / Claude Desktop\\\footnotesize 接收用户请求，转发到 MCP Server};
\node[layer] (L2) at (0, -2.0) {\textbf{第2层\quad MCP Host（协议运行时）}\quad 负责消息路由、能力发现、权限控制\\\footnotesize JSON-RPC 2.0 over stdio / SSE / HTTP};
\node[layerhot] (L1) at (0, -4.0) {\textbf{第1层\quad MCP Server（工具 / 数据接入方）}\quad amap / ifind / browser-use / excel 等千余连接器\\\footnotesize 将外部能力封装为统一 Tool Schema};

% 层间接口标签
\node[font=\footnotesize\bfseries, blue!70] at (6.5, -1.0) {Tools / Resources / Prompts（能力声明）};
\node[font=\footnotesize\bfseries, orange!80!black] at (6.5, -3.0) {JSON-RPC 请求/响应 + 通知};

% 双向箭头
\draw[bidir] ([xshift=-1cm]L3.south) -- ([xshift=-1cm]L2.north);
\draw[bidir] ([xshift=1cm]L2.south) -- ([xshift=1cm]L1.north);

% 右侧标签轴
\draw[arrow, red!70] (7.0, 0.5) -- (7.0, -4.5);
\node[font=\footnotesize\bfseries, red!70, align=center] at (8.0, -2.0) {抽象\\\\\\\\逐层\\下沉};

% 左侧标签轴
\draw[arrow, green!60!black] (-7.0, 0.5) -- (-7.0, -4.5);
\node[font=\footnotesize\bfseries, green!60!black, align=center] at (-8.0, -2.0) {接近\\用户\\\\\\\\接近\\工具};
\end{tikzpicture}
\caption{MCP 三层架构示意（自制）}
\label{fig:mcp-arch}
\end{figure}
...
\begin{tikzpicture}[
  ...
  role/.style={rectangle, rounded corners=3pt, draw=blue!70, very thick, fill=blue!8, minimum width=2.6cm, minimum height=0.9cm, align=center, font=\small\bfseries, inner sep=3pt},
  step/.style={rectangle, rounded corners=2pt, draw=orange!80, fill=yellow!15, minimum width=2.6cm, minimum height=0.7cm, align=center, font=\footnotesize, inner sep=2pt},
  arr/.style={-{Stealth[length=2.5mm]}, thick, blue!70},
  arrR/.style={-{Stealth[length=2.5mm]}, thick, orange!80},
]
% 三个角色
\node[role] (H)  at (0, 0)    {Host\\(IDE)};
\node[role] (C)  at (6, 0)    {MCP Client\\(运行时)};
\node[role] (S)  at (12, 0)   {MCP Server\\(工具源)};

% 6 步生命周期（垂直时序）
\node[step] (s1) at (0, -1.6)    {1. 用户提问};
\node[step] (s2) at (6, -1.6)    {2. 解析意图};
\node[step] (s3) at (12, -1.6)   {3. 调用 Tool};
\node[step] (s4) at (12, -3.0)   {4. 返 JSON 结果};
\node[step] (s5) at (6, -3.0)    {5. 包装为上下文};
\node[step] (s6) at (0, -3.0)    {6. 生成回答};

% 时序箭头
\draw[arr] (H.south |- s1.north) -- (s1.north);
\draw[arr] (s1.east) -- (s2.west);
\draw[arrR] (s2.east) -- (s3.west);
\draw[arrR] (s3.south) -- (s4.north);
\draw[arr] (s4.west) -- (s5.east);
\draw[arr] (s5.west) -- (s6.east);
\draw[arr] (s6.north) -- (H.south |- s6.north);

% 时间轴标签
\node[font=\footnotesize\bfseries, gray!70] at (6, -4.2) {时间 $\longrightarrow$};
\end{tikzpicture}
\caption{MCP 一次调用的完整生命周期（自制）}
\label{fig:mcp-lifecycle}
\end{figure}
...
\toprule
\textbf{组别} & \textbf{MCP名称} & \textbf{类型} & \textbf{官方/GitHub项目} & \textbf{主要能力} \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{1. 浏览器组}}} \\
\midrule
...
\midrule
\multicolumn{5}{l}{\textit{\textbf{2. 金融数据组}}} \\
\midrule
金融数据 & tushareMcp & SSE远程 & \url{https://tushare.pro/document/1?doc_id=463} & A股/港股/美股/基金/期货/期权/宏观200+接口 \\
金融数据 & china-stock-mcp & stdio & \url{https://github.com/xinkuang/china-stock-mcp} & A股行情/财务/资金流/技术指标 \\
金融数据 & yahoo-finance & stdio & \url{https://github.com/sdpa/yahoo-finance-mcp} & 全球股票/ETF/期权/加密货币 \\
金融数据 & finance-mcp & SSE远程 & \url{https://finvestai.top/mcp} & 基于Tushare的AI金融数据 \\
金融数据 & hexin-ifind-ds-stock-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD股票数据 \\
金融数据 & hexin-ifind-ds-bond-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD债券数据 \\
金融数据 & hexin-ifind-ds-fund-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD基金数据 \\
金融数据 & hexin-ifind-ds-edb-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD宏观经济数据库 \\
金融数据 & hexin-ifind-ds-index-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD指数数据 \\
金融数据 & hexin-ifind-ds-global-stock-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD全球股票数据 \\
金融数据 & hexin-ifind-ds-news-mcp & SSE远程 & \url{https://mcp.51ifind.com/} & 同花顺iFinD财经新闻 \\
金融数据 & openbb-mcp & HTTP & \url{https://github.com/OpenBB-finance/OpenBB} & 开放金融研究平台 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{3. 经济数据组}}} \\
\midrule
经济数据 & oecd-mcp & stdio & \url{https://github.com/isakskogstad/OECD-MCP} & OECD 38国经济/教育/环境数据 \\
经济数据 & worldbank-data360 & SSE远程 & \url{https://github.com/worldbank/data360-mcp} & 世界银行200+国家发展指标 \\
经济数据 & openecon-data & SSE远程 & \url{https://github.com/hanlulong/openecon-data} & 330K+指标(FRED/WB/IMF/Eurostat) \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{4. Office组}}} \\
\midrule
Office & excel & stdio & \url{https://github.com/negokaz/excel-mcp-server} & Excel读写/表格/图表/截图 \\
...
\midrule
\multicolumn{5}{l}{\textit{\textbf{5. 图表可视化组}}} \\
\midrule
图表可视化 & mcp-server-chart & stdio & \url{https://github.com/antvis/mcp-server-chart} & AntV 25+图表类型 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{6. 地图地理组}}} \\
\midrule
地图 & amap-maps & stdio & \url{https://lbs.amap.com/api/mcp-server/summary} & 高德地理编码/路径/POI/天气 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{7. PDF文献组}}} \\
\midrule
PDF & scansci-pdf & stdio & \url{https://github.com/Rimagination/scansci-pdf} & 学术论文PDF搜索/下载 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{8. 系统自动化组}}} \\
\midrule
系统 & WindowsMCP.Net & stdio & (Windows专用) & Windows原生UI操作 \\
系统 & automation-mcp & stdio & \url{https://github.com/ashwwwin/automation-mcp} & macOS键鼠/窗口/截图 \\
系统 & macos-automator & stdio & \url{https://github.com/steipete/macos-automator-mcp} & macOS AppleScript/JXA脚本 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{9. AI辅助组}}} \\
\midrule
AI辅助 & sequential-thinking & stdio & \url{https://github.com/modelcontextprotocol/servers} & 结构化逐步推理 \\
AI辅助 & context7 & stdio & \url{https://github.com/upstash/context7-mcp} & 实时库文档查询 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{10. 云部署组}}} \\
\midrule
云部署 & vercel & SSE远程 & \url{https://mcp.vercel.com} & Vercel部署/项目管理 \\
云部署 & cloudflare-api & SSE远程 & \url{https://mcp.cloudflare.com} & Cloudflare账户管理 \\
云部署 & cloudflare-builds & SSE远程 & \url{https://builds.mcp.cloudflare.com} & Cloudflare Workers构建 \\
云部署 & cloudflare-bindings & SSE远程 & \url{https://bindings.mcp.cloudflare.com} & Cloudflare KV/R2/D1绑定 \\
云部署 & cloudflare-observability & SSE远程 & \url{https://observability.mcp.cloudflare.com} & Cloudflare日志/分析 \\
云部署 & cloudflare-docs & SSE远程 & \url{https://docs.mcp.cloudflare.com} & Cloudflare文档实时查询 \\
云部署 & edgeone-makers & SSE远程 & \url{https://mcp-on-edge.edgeone.site} & 腾讯EdgeOne HTML部署 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{课程标准配置（讲师机实测版）}

把\texttt{mcp.json}内容替换为下列JSON（与本机讲师机一致）。下面同时展示本地stdio与远程SSE两种配置方式。
...
\begin{warningbox}[路径与凭证替换提醒]
\begin{itemize}[leftmargin=1em]
  \item 将\texttt{<用户名>}替换为你的Windows/macOS实际用户名
  \item \texttt{STATA\_PATH}必须与本机Stata实际安装路径一致
  \item \texttt{<YOUR\_TUSHARE\_TOKEN>}为Tushare个人Token（在 \url{https://tushare.pro} 注册后于个人中心获取）
  \item \texttt{<YOUR\_AMAP\_KEY>}为高德开放平台API Key（在 \url{https://lbs.amap.com} 申请）
  \item JSON中反斜杠必须写成\texttt{\textbackslash\textbackslash}
\end{itemize}
\end{warningbox}

\textbf{同花顺iFinD与finance-mcp等远程MCP配置}（在浏览器组SSE配置区中追加即可）：
...
\textbf{配置差异总结}：本地stdio MCP需要\texttt{command}字段指定启动命令；远程SSE MCP只需\texttt{url}字段指向服务地址。Tushare/iFinD/finance-mcp等远程服务需要额外的API Key或Token才能调用付费接口。
...
% ============================================================
\section{金融数据组MCP：投资研究数据中枢}
% ============================================================

金融数据组是本课程\textbf{最重要的新增内容}。浏览器组只能访问公开网页，调用频率高时易被反爬。金融数据组MCP通过官方授权的API（TuShare、Yahoo Finance、同花顺iFinD等）提供高频、稳定、合规的结构化金融数据，是构建可复现量化研究的基础。本节详解六个主要金融数据MCP：Tushare、同花顺iFinD系列、Yahoo Finance、China Stock、Finance-MCP、OpenBB。

\subsection{Tushare MCP}

\textbf{官方项目}：\url{https://tushare.pro/document/1?doc_id=463}（Tushare官方MCP文档）

Tushare是国内最流行的金融数据接口平台之一，提供了200+个数据接口，覆盖股票、基金、债券、期货、期权、指数、外汇、行业、宏观经济等几乎所有金融领域。Tushare MCP将所有接口以\textbf{工具函数}的形式暴露给AI，AI可以像调用Python函数一样调用Tushare的全部能力。

\subsubsection{主要数据能力清单}
...
\begin{tabular}{lp{10cm}}
\toprule
\textbf{数据类别} & \textbf{主要工具（示例）} \\
\midrule
股票基础信息 & \texttt{stock\_basic}（股票列表）、\texttt{stock\_company}（公司概况） \\
行情数据 & \texttt{daily}（日线）、\texttt{weekly}（周线）、\texttt{monthly}（月线） \\
财务数据 & \texttt{income}（利润表）、\texttt{balancesheet}（资产负债表）、\texttt{cashflow}（现金流量表） \\
财务指标 & \texttt{fina\_indicator}（主要财务指标）、\texttt{fina\_audit}（审计意见） \\
资金流向 & \texttt{moneyflow}（个股资金流）、\texttt{moneyflow\_hsgt}（沪深港通资金） \\
股东与机构 & \texttt{top10\_holders}（前十大股东）、\texttt{top\_inst}（龙虎榜机构） \\
基金 & \texttt{fund\_basic}（基金列表）、\texttt{fund\_nav}（基金净值） \\
指数 & \texttt{index\_basic}（指数列表）、\texttt{index\_daily}（指数日线） \\
期货与期权 & \texttt{fut\_basic}、\texttt{fut\_daily}、\texttt{opt\_daily} \\
宏观与利率 & \texttt{shibor}、\texttt{libor}、\texttt{cn\_gdp}、\texttt{cn\_cpi}、\texttt{cn\_pmi} \\
新闻公告 & \texttt{news}、\texttt{anns\_d}（公告查询） \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{L1任务提示词：股票多因子分析}
...
\begin{theorybox}[Tushare积分制与Token管理]
Tushare采用积分制管理API调用权限：注册用户默认100积分，可调用基础接口；高级接口（如分钟线、龙虎榜）需要5000以上积分。Token可在Tushare官网个人中心获取，配置到MCP的\texttt{url}字段中。每个Token对应一个用户身份，**请勿在公开仓库中泄露Token**。
\end{theorybox}

\subsection{同花顺iFinD系列MCP}

\textbf{官方项目}：\url{https://mcp.51ifind.com/}（同花顺iFinD官方MCP服务）

同花顺iFinD是国内金融机构使用最广泛的金融数据终端之一。2026年3月，同花顺正式推出iFinD MCP服务（\textit{《iFinD金融MCP正式上线，给OpenClaw们配上专业的金融数据》}，新浪财经报道），将iFinD的7大数据子库通过MCP协议开放给AI智能体：
...
\toprule
\textbf{MCP名称} & \textbf{对应数据子库} & \textbf{核心能力} \\
\midrule
\texttt{hexin-ifind-ds-stock-mcp} & 股票 & A股/港股/美股行情、财务数据、估值指标、股东信息 \\
\texttt{hexin-ifind-ds-bond-mcp} & 债券 & 利率债、信用债、可转债行情与估值 \\
\texttt{hexin-ifind-ds-fund-mcp} & 基金 & 公募/私募基金净值、持仓、业绩归因 \\
\texttt{hexin-ifind-ds-edb-mcp} & 宏观数据库 & GDP/CPI/PMI/M2/社融/信贷等宏观指标 \\
\texttt{hexin-ifind-ds-index-mcp} & 指数 & 中证/国证/申万等行业指数与风格指数 \\
\texttt{hexin-ifind-ds-global-stock-mcp} & 全球股票 & 美股/港股/欧股/日股基本面与行情 \\
\texttt{hexin-ifind-ds-news-mcp} & 财经新闻 & 实时财经新闻、公告、研报摘要 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{L2任务提示词：债券信用利差分析}
...
\subsection{Yahoo Finance MCP}

\textbf{GitHub项目}：\url{https://github.com/sdpa/yahoo-finance-mcp}

Yahoo Finance是全球最全面的免费金融数据源之一，覆盖美股、港股、欧股、日股、ETF、期权、期货、外汇、加密货币。Yahoo Finance MCP基于\texttt{yahoo-finance2}库，通过非官方API获取数据（与Tushare/iFinD的官方授权不同，Yahoo Finance数据**仅供学术研究使用**）。
...
\begin{tabular}{ll}
\toprule
\textbf{数据类别} & \textbf{代表工具} \\
\midrule
股票行情 & \texttt{get\_stock\_summary}、\texttt{get\_stock\_info}、\texttt{get\_stock\_performance} \\
财务报表 & \texttt{get\_stock\_financials} \\
股东信息 & \texttt{get\_stock\_shareholders} \\
风险指标 & \texttt{get\_risk\_indicators} \\
事件日历 & \texttt{get\_stock\_events} \\
ESG数据 & \texttt{get\_esg\_data} \\
高频行情 & \texttt{stock\_highfreq\_quotes} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{L4任务提示词：中美银行股估值对比}
...
\subsection{China Stock MCP}

\textbf{GitHub项目}：\url{https://github.com/xinkuang/china-stock-mcp}

China Stock MCP是基于\texttt{akshare}库的A股专业数据MCP，提供行情、财务、资金流、技术指标、股东户数、龙虎榜等A股特色数据。与Tushare的差异在于：China Stock完全免费且无需Token，Tushare高阶接口需要积分但数据更稳定。
...
股东户数 & A股股东户数变化趋势 \\
技术指标 & MACD/KDJ/BOLL等通过\texttt{stock\_technical\_rank}直接调用 \\
\bottomrule
\end{tabular}
\end{table}

\begin{notebox}[Tushare与China Stock的互补策略]
\textbf{策略1}：日常行情监控用China Stock（免费、高频、实时），财务深度分析用Tushare（稳定、字段全、含历史财报）；

\textbf{策略2}：龙虎榜、涨停板、连板等A股特色数据用China Stock（Tushare无对应接口）；

\textbf{策略3}：分钟级回测用Tushare+积分制（2000积分以上可用1/5/15/30/60分钟线），低频分析用China Stock。
\end{notebox}

\subsection{Finance-MCP}

\textbf{官方服务}：\url{https://finvestai.top/mcp}

Finance-MCP是FinvestAI团队基于Tushare API封装的金融数据MCP，与Tushare MCP的区别在于：Finance-MCP将多个相关接口组合为\textbf{复合工具}（如\texttt{get\_stock\_fundamental\_analysis}一次性返回估值、财务、机构持仓），适合AI直接产出\textbf{结构化研报}而非原始数据。
...
\subsection{OpenBB MCP}

\textbf{GitHub项目}：\url{https://github.com/OpenBB-finance/OpenBB}（OpenBB官方）

\textbf{PyPI}：\url{https://pypi.org/project/openbb-mcp-server/}

OpenBB是开源金融研究平台的代表项目，前身为Gamestonk Terminal。OpenBB MCP通过MCP协议暴露了OpenBB Platform的全部REST API，覆盖股票、期权、期货、外汇、加密货币、经济数据、新闻等。OpenBB的优势在于\textbf{数据源聚合}——同一指标可从Yahoo、FRED、SEC、Quandl等多个源获取，并提供交叉验证。

\begin{notebox}[金融数据MCP的选型决策树]
需要\textbf{官方授权、稳定、合规}的金融数据 → \textbf{Tushare MCP}（A股为主）或\textbf{同花顺iFinD}（机构级数据）

需要\textbf{全球跨市场}数据（美股/港股/欧股） → \textbf{Yahoo Finance MCP}

需要\textbf{A股特色数据}（龙虎榜/涨停板/资金流） → \textbf{China Stock MCP}

需要\textbf{复合研报数据}（一次返回多维度） → \textbf{Finance-MCP}

需要\textbf{多源交叉验证} → \textbf{OpenBB MCP}
\end{notebox}

% ============================================================
\section{经济数据组MCP：宏观研究数据源}
% ============================================================
...
\textbf{GitHub项目}：\url{https://github.com/isakskogstad/OECD-MCP}

\textbf{npm包}：\url{https://www.npmjs.com/package/oecd-mcp}

OECD MCP由社区开发者isakskogstad维护，封装了OECD的SDMX API，覆盖38个成员国及部分非成员国的经济、教育、健康、环境、就业、金融等17个类别的\textbf{1500+数据集}。
...
\begin{tabular}{lll}
\toprule
\textbf{数据集代码} & \textbf{名称} & \textbf{金融研究应用} \\
\midrule
\texttt{EO} & Economic Outlook & GDP增速、通胀预测 \\
\texttt{KEI} & Key Economic Indicators & 利率、失业率、贸易差额 \\
\texttt{SNA\_TABLE1} & GDP by Industry & 三次产业结构分析 \\
\texttt{CPI} & Consumer Prices & 跨国通胀比较 \\
\texttt{IRLT} & Long-term Interest Rates & 主权债收益率 \\
\texttt{BSCI} & Bank Soundness Indicators & 跨国银行稳健性指标 \\
\texttt{FDI} & FDI Flows & 跨境投资分析 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{L5任务提示词：跨国银行业稳健性对比}
...
\subsection{World Bank Data360 MCP}

\textbf{GitHub项目}：\url{https://github.com/worldbank/data360-mcp}（世界银行官方）

\textbf{官方文档}：\url{https://worldbank.github.io/data360-mcp/}

World Bank Data360是世界银行在2024年推出的新一代数据平台，整合了World Development Indicators、International Debt Statistics、Doing Business等10+个数据库。Data360 MCP是世界银行官方维护的MCP服务，提供\textbf{200+国家、5000+指标}的访问。

\subsubsection{核心能力}

\begin{itemize}[leftmargin=2em]
  \item \texttt{data360\_search\_indicators}：按关键词搜索指标（如"financial inclusion"）
  \item \texttt{data360\_search\_datasets}：搜索数据集
  \item \texttt{data360\_get\_data}：获取指标时间序列数据
  \item \texttt{data360\_rank\_countries}：跨国排名
  \item \texttt{data360\_compare\_countries}：多国对比
  \item \texttt{data360\_summarize\_data}：数据汇总统计
\end{itemize}

\subsubsection{L6任务提示词：全球金融包容性分析}
...
\subsection{OpenEcon Data MCP}

\textbf{GitHub项目}：\url{https://github.com/hanlulong/openecon-data}

\textbf{官方网站}：\url{https://openecon.ai}

OpenEcon Data MCP是hanlulong（同时也是stata-mcp作者）开发的经济数据聚合服务，将\textbf{10+数据源}统一为一个MCP接口：
...
\begin{theorybox}[金融数据MCP vs 经济数据MCP的使用边界]
\textbf{金融数据MCP}（Tushare/iFinD/Yahoo）：以\textbf{微观主体}（上市公司/银行/基金/个股）为对象，提供高频、精细化的市场与财务数据，适合\textbf{投资研究}、\textbf{资产定价}、\textbf{公司金融}等场景。

\textbf{经济数据MCP}（OECD/WB/OpenEcon）：以\textbf{宏观经济变量}（GDP/CPI/利率/汇率/就业）为对象，提供中低频、跨国可比的数据，适合\textbf{宏观研究}、\textbf{货币政策分析}、\textbf{国际金融}等场景。

\textbf{协同范式}：宏观研究通常需要"自上而下"——先用经济数据MCP分析宏观周期与货币政策，再用金融数据MCP分析行业与个股的传导效应。
\end{theorybox}
...
% ============================================================
\section{图表可视化MCP：AntV Chart金融图表生成}
% ============================================================

\textbf{GitHub项目}：\url{https://github.com/antvis/mcp-server-chart}（AntV官方，4.2k stars）

mcp-server-chart是蚂蚁集团AntV团队开发的可视化MCP，基于AntV开源可视化库（G2/G6/L7等），提供\textbf{25+种图表类型}的代码生成与渲染能力。在金融场景中，AntV Chart MCP弥补了Excel MCP图表类型有限、定制化困难的不足。
...
\begin{theorybox}[AntV Chart MCP vs Excel MCP图表能力对比]
\textbf{Excel MCP}：擅长\textbf{数据驱动的标准图表}，与Excel深度集成，适合做日常报表。其图表嵌入在xlsx文件中，可与数据联动更新。图表类型覆盖基础10种（柱/折线/饼/散点/面积/雷达/3D等）。

\textbf{AntV Chart MCP}：擅长\textbf{定制化的高级可视化}，输出独立PNG/SVG文件，适合做研报插图、演示文稿、海报设计。图表类型丰富（25+种），样式高度可定制（颜色/字体/标注/交互）。

\textbf{协同策略}：日常数据探索用Excel MCP，研报/PPT中的关键图表用AntV Chart MCP精雕细琢。
\end{theorybox}
...
\subsection{高德地图 MCP（amap-maps）}

\textbf{官方项目}：\url{https://lbs.amap.com/api/mcp-server/summary}（高德地图官方）

\textbf{npm包}：\url{https://www.npmjs.com/package/@amap/amap-maps-mcp-server}
...
\subsection{Scansci PDF MCP}

\textbf{GitHub项目}：\url{https://github.com/Rimagination/scansci-pdf}

Scansci PDF是面向研究者与学生的学术论文PDF下载MCP，支持DOI、arXiv ID、URL、论文标题四种输入方式。Scansci-PDF采用\textbf{MCP+Skill双层架构}：MCP层提供21个底层工具（来源查询、PDF下载、批量处理、Zotero联动等），Skill层提供"如何找到一篇好论文"的工作流模板。
...
% ============================================================
\section{系统自动化与AI辅助MCP}
% ============================================================

\subsection{macOS桌面自动化双雄：automation-mcp 与 macos-automator}

\subsubsection{automation-mcp}

\textbf{GitHub项目}：\url{https://github.com/ashwwwin/automation-mcp}

automation-mcp由ashwwwin开发，提供macOS系统级UI自动化能力：鼠标控制、键盘输入、窗口管理、屏幕截图、UI元素识别。其设计理念是\textbf{让AI像人一样操作Mac}，适用于没有专用MCP的桌面软件（如老版网银客户端、行情软件等）。
...
\subsubsection{macos-automator}

\textbf{GitHub项目}：\url{https://github.com/steipete/macos-automator-mcp}

\textbf{npm包}：\url{https://www.npmjs.com/package/@steipete/macos-automator-mcp}
...
\begin{theorybox}[automation-mcp vs macos-automator的能力边界]
\textbf{automation-mcp}：通用型UI自动化，通过屏幕坐标+像素识别操作UI元素。优势：\textbf{无需目标软件提供API}，任何macOS应用都可操作。劣势：依赖视觉识别，对动态UI（如动画、滚动）鲁棒性较差。

\textbf{macos-automator}：API型系统集成，通过执行AppleScript/JXA脚本直接调用macOS API。优势：\textbf{执行效率高、稳定性强}，可批量处理系统级任务。劣势：仅适用于支持AppleScript的应用（Finder/Safari/Mail/Notes等原生应用支持好，第三方应用支持参差不齐）。

\textbf{协同范式}：优先用macos-automator处理原生应用（如Finder批量重命名），回退到automation-mcp处理第三方应用UI。
\end{theorybox}

\subsection{Sequential Thinking MCP}

\textbf{GitHub项目}：\url{https://github.com/modelcontextprotocol/servers}（MCP官方reference server中的\texttt{sequentialthinking}）

Sequential Thinking是MCP协议作者Anthropic官方维护的reference server之一，提供\textbf{结构化逐步推理}能力。与普通AI对话不同，Sequential Thinking强制AI将复杂问题分解为多个思考步骤，每步可修订、追溯、分支，特别适合金融分析中的多步推理（如"先看宏观、再看行业、再看个股"的研究框架）。
...
\subsection{Context7 MCP}

\textbf{GitHub项目}：\url{https://github.com/upstash/context7-mcp}

\textbf{npm包}：\url{https://www.npmjs.com/package/@upstash/context7-mcp}

Context7由Upstash（知名云服务厂商）维护，提供\textbf{实时库文档查询}能力。当AI需要使用某个Python/JS/Go库的API时，Context7可以返回该库的最新文档（而非AI训练数据中的过时信息）。在金融数据MCP使用中，Context7可以帮AI快速查Tushare接口的最新字段定义、AntV Chart的最新图表参数等。
...
以下是一个典型的银行分析全链路工作流，展示了\textbf{金融数据 + Excel + AntV Chart + Word + PPT}五MCP协同：

\textbf{任务描述}：为某上市银行自动生成季度经营分析报告。

\textbf{Step 1 — 数据采集}（Tushare + iFinD + OECD 金融数据组MCP）
...
\textbf{Step 3 — 高级可视化}（AntV Chart MCP）
...
\textbf{Step 6 — 一键部署}（Vercel/Cloudflare MCP，可选）
...
\subsection{“宏观—行业—个股”三步式金融研究工作流}
...
这一案例完整覆盖了「宏观-行业-个股」三层研究框架，是金融从业者使用MCP实现\textbf{可复现量化研究}的典型范式。
...
\keyterms{
MCP（Model Context Protocol）、MCP Client、MCP Server、JSON-RPC 2.0、Tool Schema、工具发现、stdio传输、SSE传输、Streamable HTTP、Playwright MCP、Lighthouse审计、Fetch MCP、Tushare MCP、同花顺iFinD MCP、Yahoo Finance MCP、China Stock MCP、OpenBB、OECD MCP、World Bank Data360 MCP、OpenEcon Data MCP、Excel MCP、Word MCP、PowerPoint MCP、AntV Chart MCP、高德地图 MCP、Scansci PDF MCP、automation-mcp、macos-automator、Sequential Thinking MCP、Context7 MCP、Vercel MCP、Cloudflare MCP、EdgeOne MCP、面板数据固定效应模型
}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{金融数据MCP选型题}：你需要完成一项“上市银行顺周期能力”研究。
  \begin{itemize}
    \item 该研究中需要拉取36家上市银行2015-2024年财务数据，应选择哪个MCP？为什么不直接使用Fetch MCP抓报表？
    \item 跨国对比阶段需要与G7银行体系比较，应选择哪个经济数据MCP？该MCP的官方项目地址是什么？
    \item 研究报告需要一张高质量的“股价叠加 vs 宏观周期”多变量图，应使用Excel MCP还是AntV Chart MCP？为什么？
  \end{itemize}
  \item \textbf{多MCP协同实操题}：请设计一个MCP组合编排工作流，实现“使用公开论文PDF与金融数据MCP，自动生成某上市公司同业对标报告”。要求：
  \begin{itemize}
    \item 列出需要使用的MCP组合及调用顺序（需要用到 Scansci PDF MCP）
    \item 写出关键步骤的提示词框架
    \item 设计错误处理与回退策略
    \item 说明为什么需要 PDF 文献组与金融数据组 MCP 联动
  \end{itemize}
  \item \textbf{创新设计题}：请设计一个mcp组合，在「宏观周期判断」阶段调用经济数据组MCP，在「行业与个股」阶段调用金融数据组MCP，在「可视化」阶段调用AntV Chart MCP，在「一键报告」阶段调用Word/PPT/Vercel MCP。要求：
  \begin{itemize}
    \item 画出调用流程图（可用mermaid语法）
    \item 说明每一步的输入、输出与中间产物保存路径
  \end{itemize}
\end{enumerate}
\end{exercisebox}
