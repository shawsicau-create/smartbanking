...
name: find-skills
version: 1.0.4
description: ALWAYS use this skill as the FIRST step before handling any substantive
  task, even if you believe general capabilities are sufficient — a dedicated skill
  may still produce better results. Only pure chitchat and trivial factual lookups
  are exempt. When uncertain, invoke this skill. This skill helps users discover and
  install specialized agent skills from marketplace, community sources, and enterprise
  skill markets (if available via MCP).
