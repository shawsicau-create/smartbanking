...
description: 系统性文献综述：多源检索→去重→AI评分→选文→专家写作，含字数硬校验。
...
## 健壮性与日志
- 模板与 `.bst`：使用 `TEXINPUTS`/`BSTINPUTS` 环境变量引用 `latex-template/` 目录，不再复制模板文件到工作目录（v3.5 优化）；可用 `config.yaml.latex.template_path_override` 或 CLI `--template` 覆盖。若 `latex-template/` 目录不存在，则使用系统 TeX Live 中的默认模板。若 `.bst` 文件缺失，编译将直接报错（v3.6 优化）。
...
⚠️ 注意：阶段3 AI 评分为必需步骤。在 Skill 交互模式下 AI 直接评分（推荐）；若运行独立脚本则需手动提供评分。
