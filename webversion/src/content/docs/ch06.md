---
title: '第6章 金融数据分析与计量经济学'
description: '金融数据处理、面板回归、时间序列、因果推断'
---
...
<div class="chapter-audio-player" style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:linear-gradient(135deg,#f0f9ff,#e0f2fe);border:1px solid #bae6fd;border-radius:12px;margin-bottom:24px">
  <button id="ch06-play-btn" onclick="toggleChapterAudio('ch06')" style="width:40px;height:40px;border:none;border-radius:50%;background:linear-gradient(135deg,#3B82F6,#2563EB);color:white;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0">▶</button>
  <div style="flex:1;min-width:0">
    <div style="font-weight:600;font-size:14px;color:#0f172a">📢 本章语音导览</div>
    <audio id="ch06-audio" preload="none" style="width:100%;height:32px;margin-top:4px">
      <source src="/audio/ch06_narration.mp3" type="audio/mpeg">
    </audio>
  </div>
  <a href="/slides/#ch06" style="font-size:13px;color:#2563EB;text-decoration:none;white-space:nowrap">🖼️ 查看课件 →</a>
</div>
...
#### 场景A：AI IDE已预装BMAD（如Qoder） {#场景aai-ide已预装bmad如qoder .unnumbered}
...
#### 场景B：AI IDE未预装BMAD（如Cursor、Claude Code） {#场景bai-ide未预装bmad如cursorclaude-code .unnumbered}
...
#### Sprint 2新增模块 {#sprint-2新增模块 .unnumbered}
...
#### Sprint 3新增模块 {#sprint-3新增模块 .unnumbered}
...
#### 前置准备：创建EdgeOne Pages项目 {#前置准备创建edgeone-pages项目 .unnumbered}

访问<https://console.cloud.tencent.com/edgeone/pages>，点击「创建项目」，选择「直接上传」类型，上传任意文件完成项目创建。记住项目名称（如`smartbanking`）。

#### 获取API Token {#获取api-token .unnumbered}

访问<https://pages.edgeone.ai/document/api-token>，点击「Create Token」，填写名称和范围后生成Token。**立即复制**生成的Token（仅显示一次）。

#### 安装EdgeOne CLI {#安装edgeone-cli .unnumbered}
...
::: notebox
如果出现`EPERM`或`EACCES`错误，以管理员身份打开PowerShell后重试，或使用`npx edgeone`替代全局安装。
:::

#### 部署前端项目 {#部署前端项目 .unnumbered}
...
首次部署时，EdgeOne CLI会自动将`dist/`目录下的所有静态文件上传到EdgeOne CDN。
...
### 多平台部署扩展：Cloudflare Pages

除了EdgeOne Pages，本课程还支持将网站部署到**Cloudflare Pages**------Cloudflare提供的全球CDN静态网站托管服务。Cloudflare Pages的优势在于全球节点覆盖广、免费额度充足，且提供`wrangler` CLI工具实现命令行直传部署。

#### 安装wrangler CLI {#安装wrangler-cli .unnumbered}
...
#### 登录Cloudflare {#登录cloudflare .unnumbered}

``` {style="shell"}
wrangler login
```

执行后浏览器会自动打开授权页面，点击「Allow」完成OAuth授权。随后通过`wrangler whoami`验证登录状态。

#### 配置wrangler.toml {#配置wrangler.toml .unnumbered}

在项目根目录创建`wrangler.toml`文件，声明Pages项目的输出目录：
...
#### 构建与部署 {#构建与部署 .unnumbered}

由于Astro需要根据部署平台动态生成sitemap和canonical URL，构建时需设置`CLOUDFLARE_PAGES`环境变量：
...
#### 三平台部署对比 {#三平台部署对比 .unnumbered}

本课程支持三种静态网站部署平台，各有优劣：

  **平台**           **访问域名**      **国内速度**   **特点**
  ------------------ ----------------- -------------- ---------------------------------------
  EdgeOne Pages      `*.edgeone.app`   快             腾讯云CDN，中国大陆加速需控制台启用
  Cloudflare Pages   `*.pages.dev`     中             全球节点覆盖最广，wrangler CLI直传
  GitHub Pages       `*.github.io`     慢             与Git仓库集成，GitHub Actions自动部署
...
5.  CNB的OAuth登录和PAT令牌两种认证方式，各有什么优缺点？如果要将后端也部署到云端（而非仅前端部署EdgeOne Pages），有哪些方案？
