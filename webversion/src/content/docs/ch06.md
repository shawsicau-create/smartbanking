...
**标准化提示词模板**（实验报告附件用）：
    ...
    输入数据：<文件路径 + 关键变量>
    输出要求：<文件路径 + 格式>
    环境检查：请先运行 /tmp/check_ch06_env.sh
    原代码补充：<可选，保留关键代码块>
...
``` {style="shell"}
stata-mp -b do chapter6_panel.do    # 输出 chapter6_panel.log
```
...
本节是ch06所有用到的Skill环境检查总表，覆盖约30个Skill，每个Skill列出安装指令、环境检查命令、在ch06中出现的位置。
...
### ch06全章一键环境检查脚本（期末上机前必运行）

``` {style="shell"}
cat > /tmp/check_ch06_env.sh << 'EOF'
#!/bin/bash
PASS=0; FAIL=0
...
[ \$FAIL -eq 0 ] && echo "OK ch06环境就绪" || echo "请安装缺失组件后再进行实验"
EOF
chmod +x /tmp/check_ch06_env.sh && /tmp/check_ch06_env.sh
```
...
**缺失值处理（Skill 化）**：
    ...
    【环境检查】运行 /tmp/check_ch06_env.sh 确认 stata-mp 可访问
...
// Step 4: 输出三列回归表
esttab ols fe re using reports/ch06_panel_reg.rtf, ///
...
**提示词**：
    ...
    【第 4 步】结果输出
    1. 使用 esttab 命令输出三列回归结果到 reports/ch06_panel_reg.txt
...
**三模型面板回归（Stata Skill 化）**：
    ...
    【输入】data/bank_panel.dta
    【输出要求】
      - reports/ch06_panel_reg.tex（三列回归表 LaTeX 代码）
      - reports/ch06_panel_reg.pdf（编译后 PDF）
      - figures/coefficient_compare.pdf（系数对比图）
    【环境检查】运行 /tmp/check_ch06_env.sh
    【原代码补充】Stata 代码（作为补充）：
      reg roe npl_ratio car loan_growth log_asset, robust
      est store ols
      reghdfe roe npl_ratio car loan_growth log_asset, absorb(bank_id year) robust
      ...
      est store re
      hausman fe2 re
      esttab ols fe2 re using ch06_panel_reg.tex, b(3) se(3) star(* 0.1 ** 0.05 *** 0.01)
...
**R fixest 多维 FE（R Skill 化）**：
    ...
    【输出要求】
      - reports/ch06_fixest_reg.md（含 R 输出表格）
      - reports/ch06_fixest_reg.tex（modelsummary 输出的 LaTeX 代码）
    ...
    【原代码补充】R 代码：
      ...
      modelsummary(list("FE" = reg1), output = "ch06_fixest.tex", stars = TRUE)
...
**动态面板 GMM（Python Skill 化）**：
    ...
    【输出】
      - reports/ch06_gmm_reg.md（含 GMM 估计系数表）
...
**回归系数对比可视化（econ-visualization Skill 化）**：
    ...
    【输入】reports/ch06_panel_reg.tex 中的系数表
...
**提示词**：
    ...
    保存预测图到 figures/ch06_forecast_compare.png
...
**提示词示例**：
    ...
    5. 输出：figures/ch06_event_study.pdf + ch06_roe_compare.pdf
...
**提示词示例**：
    ...
    - 输入：R回归对象 ols/fe/re
    - 输出：ch06_table1.tex（含三线表、t统计量、星号标注、模型标题）
...
**改写对照表**（三种场景）：
  ...
  **场景**      **原提示词**          **Skill化后提示词**
  ------------- --------------------- -----------------------------------------------------------------------------------------------------------------------------------------------------
  Stata回归     \"跑reghdfe命令\"     \"请调用 `stata-regression` Skill跑reghdfe，使用 y、i.firm、c.x1##c.x2、fe(year)，vce(cluster firm)，输出三列回归表到 ch06_table1.tex\"
...
**Skill 调用记录文件**：实验结束后必须生成 `reports/ch06_skill_workflow.md`（完整 Skill 调用日志），包含：每次调用的 Skill 名称、输入输出文件、执行时长、错误信息与回退方案。该文件作为实验报告附件提交，是 Skill 化实验的核心凭证。
...
**【Skill 化版】调用 `data-analysis` + `econ-visualization`**：
    ...
    【输出要求】
      - reports/ch06_desc_stat.md（含均值、标准差、分位数、相关系数矩阵）
      - reports/ch06_desc_stat.tex（LaTeX 表格，latex-tables Skill 输出）
      - figures/ch06_distribution.pdf（各变量分布直方图与正态性检验）
      - figures/ch06_correlation_heatmap.pdf（核心变量相关矩阵热力图）
    【环境检查】运行 /tmp/check_ch06_env.sh；python3 -c "import seaborn, matplotlib"
...
**【原裸代码提示词·保留】**：
    ...
    3. 计算核心变量的相关系数矩阵
    4. 绘制 ROE 的直方图，检验正态性
    5. 输出描述统计表和相关系数矩阵到 reports/ch06_desc_stat.txt
...
**【Skill 化版】调用 `stata-regression` + `latex-tables`**：
    ...
    【输入数据】
      ...
      - 控制变量：LOAN、GDP、CPI
    【输出要求】
      - reports/ch06_baseline.tex（三线表，latex-tables Skill 输出，含3 列嵌套模型）
      - reports/ch06_baseline.pdf（paper-compile Skill 编译生成）
      - figures/ch06_baseline_coefplot.pdf（系数对比图，econ-visualization 输出）
...
**【原裸代码提示词·保留】**：
    ...
    2. 同时估计三个嵌套模型：
       ...
       - 模型2：加入控制变量
       - 模型3：加入年份固定效应
    3. 输出三列回归表到 reports/ch06_baseline.rtf
...
**【Skill 化版】调用 `stata-regression` + `econ-visualization` + `paper-figure`**：
    ...
    【输出要求】
      - reports/ch06_robustness.tex（4 列稳健性表格：替换 ROA / Winsorize / 排除 2020 / 滞后项）
      - reports/ch06_robustness.pdf
      - figures/ch06_robustness_forest.pdf（稳健性系数对比森林图，econ-visualization）
      - figures/ch06_robustness_sensitivity.pdf（敏感性分析图，paper-figure）
...
**【原裸代码提示词·保留】**：
    ...
    保存到 reports/ch06_robustness.rtf
...
**【Skill 化版】调用 `data-analysis` + `econ-visualization`**：
    ...
    【输出要求】
      - reports/ch06_heterogeneity.tex（按银行类型与区域分组回归表）
      - reports/ch06_heterogeneity.pdf
      - figures/ch06_heterogeneity_type.pdf（4 类银行系数对比图）
      - figures/ch06_heterogeneity_region.pdf（3 大区域对比热力图）
      - reports/ch06_chow_test.md（Chow 检验结果汇总）
...
**【原裸代码提示词·保留】**：
    ...
    输出分组回归表到 reports/ch06_heterogeneity.rtf
...
**【Skill 化版】调用 `r-econometrics` 中介模块**：
    ...
    【输出要求】
      - reports/ch06_mediation.tex（Sobel 检验 + Bootstrap 置信区间表）
      - reports/ch06_mediation.pdf
      - figures/ch06_mediation_path.pdf（中介路径示意图，econ-visualization）
      - reports/ch06_mediation_summary.md（中介效应占总效应比例）
...
**【原裸代码提示词·保留】**：
    ...
    输出中介效应检验结果到 reports/ch06_mediation.txt
...
**【Skill 化交付物清单】** 所有交付物同时输出 LaTeX + PDF 格式（除原始 .rtf/.txt 外）：
   ...
   **7**   **【新增·核心交付物】Skill 调用记录**   ---          `reports/ch06_skill_workflow.md`
   **8**   **【新增】配套图表集**                  ---          `figures/ch06_*.pdf`（5-8 张）
   **9**   **【新增】Skill 化实验报告模板**        ---          `reports/ch06_report.tex`

**`ch06_skill_workflow.md` 模板**：
...
- 调用 Skill: data-analysis, econ-visualization
- 输入: data/bank_panel_clean.csv
- 输出: reports/ch06_desc_stat.md, .tex, figures/ch06_distribution.pdf
- 执行时长: <X 秒>
- 错误与回退: <无 / 描述问题与解决方案>
...
**交付物文件清单（期末提交检查表）**：
  ...
  -- --------------------------------------- --------------------
     ...
     `reports/ch06_desc_stat.tex/.pdf`       描述统计
     `reports/ch06_baseline.tex/.pdf`        基准回归
     `reports/ch06_robustness.tex/.pdf`      稳健性
     `reports/ch06_heterogeneity.tex/.pdf`   异质性
     `reports/ch06_mediation.tex/.pdf`       中介效应
     `figures/ch06_*.pdf`                    5-8 张配套图
     `reports/ch06_skill_workflow.md`        **Skill 调用日志**
     `reports/ch06_report.tex/.pdf`          完整实验报告
...
本章系统介绍了金融数据分析与计量经济学的核心方法。6.1节阐述了金融数据的六大类型和四大典型特征（尖峰厚尾、异方差、序列相关、结构突变），以及缺失值处理、异常值检测和变量变换的完整预处理流程。6.2节深入讲解了面板数据回归的三种经典模型（混合OLS、固定效应、随机效应）及其选择检验（F检验、Hausman检验、BP-LM检验），并介绍了动态面板GMM方法。6.3节从时间序列分解出发，依次介绍了SARIMA、LSTM和Prophet三种预测方法。6.4节阐述了因果推断的四种主流方法（DID、RDD、IV、SCM）。6.5节构建了AI辅助计量分析的六步工作流，并警示了过拟合、数据窥探、伪回归等常见陷阱。6.6节通过银行盈利能力影响因素的完整实验，将上述方法串联为论文级实证研究。
