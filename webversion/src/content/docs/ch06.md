...
其中，**面板数据**是金融实证研究中最常用的数据类型，它同时包含个体异质性和时间动态性，信息量远大于纯截面或纯时序数据。本节后续内容及第[1.2](#sec:panel){reference-type="ref" reference="sec:panel"}节将重点围绕面板数据展开。
...
- **Box-Cox变换**：$y^{(\lambda)} = (y^\lambda - 1)/\lambda$，λ由极大似然估计确定
...
2.  **更多信息量**：$N$个个体× $T$期提供$NT$个观测值，增大自由度，提高估计效率
...
其中\thetâ为变换参数，取值介于0（OLS）和1（FE）之间。
...
:::tip[理论要点]
1.  **第一步**：BP-LM检验 → $p$值大？⇒ 个体效应不存在 ⇒ 用**混合OLS**

2.  **第二步**：BP-LM显著 → 个体效应存在 → 进入Hausman检验

3.  **第三步**：Hausman检验 → $p$值小？⇒ 个体效应与解释变量相关 ⇒ 用**固定效应（FE）**

4.  **第四步**：Hausman不显著 → 个体效应与解释变量不相关 ⇒ 用**随机效应（RE）**（效率更高）
