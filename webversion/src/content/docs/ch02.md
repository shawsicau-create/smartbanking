...
<div class="chapter-audio-player" style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:linear-gradient(135deg,#f0f9ff,#e0f2fe);border:1px solid #bae6fd;border-radius:12px;margin-bottom:24px">
  <button id="ch02-play-btn" onclick="toggleChapterAudio('ch02')" style="width:40px;height:40px;border:none;border-radius:50%;background:linear-gradient(135deg,#3B82F6,#2563EB);color:white;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0">▶</button>
  <div style="flex:1;min-width:0">
    <div style="font-weight:600;font-size:14px;color:#0f172a">📢 本章语音导览</div>
    <audio id="ch02-audio" preload="none" style="width:100%;height:32px;margin-top:4px">
      <source src="/audio/ch02_narration.mp3" type="audio/mpeg">
    </audio>
  </div>
  <a href="/slides/#ch02" style="font-size:13px;color:#2563EB;text-decoration:none;white-space:nowrap">🖼️ 查看课件 →</a>
</div>
...
<figcaption>AI IDE 三阶段演进示意（自制）</figcaption>
...
- **TRAE**（字节跳动）：[https://www.trae.cn/ide/download](https://www.trae.cn/ide/download)
- **Qoder**（阿里云）：[https://qoder.com/zh/download](https://qoder.com/zh/download)
- **CodeBuddy**（腾讯云）：[https://www.codebuddy.cn/ide/](https://www.codebuddy.cn/ide/)
  :::
...
CNB（CodeNotebook，[https://cnb.cool](https://cnb.cool)）是国内领先的云原生代码托管与开发平台，提供 Git 代码托管、CI/CD 流水线、AI 辅助编程等功能。相比 GitHub，CNB 具有国内访问速度快、中文界面友好、集成 AI Skill 等优势。

**注册与配置**

1. 访问 [https://cnb.cool](https://cnb.cool)，使用微信扫码注册/登录
...
1. 访问 [https://miktex.org/download](https://miktex.org/download)，下载 Basic MiKTeX Installer（约 230 MB）
...
flowchart TB
    ...
    subgraph Chat["Chat模式"]
        C1["侧边栏对话"] --> C2["仅回答"]
        C2 --> C3["最低风险"]
    end
  
    subgraph Builder["Builder模式"]
        B1["Ctrl+U"] --> B2["读写文件+执行命令"]
        B2 --> B3["中风险"]
    end
  
    subgraph SOLO["SOLO模式"]
        S1["独立面板"] --> S2["全自主操作"]
        S2 --> S3["高风险"]
    end
...
::: notebox
如果远程仓库已存在，使用 `git remote set-url origin` 更新地址。推送成功后访问 [https://cnb.cool/组织名/仓库名](https://cnb.cool/%E7%BB%84%E7%BB%87%E5%90%8D/%E4%BB%93%E5%BA%93%E5%90%8D) 查看代码。
:::
