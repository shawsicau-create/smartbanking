...
把`mcp.json`内容替换为下列JSON（与本机讲师机一致）：
...
```json
{
  "mcpServers": {
    ...
    "WindowsMCP.Net": {
      "command": "C:<br/>Users<br/><用户名><br/>.dotnet<br/>tools<br/>Windows-MCP.Net.exe",
      "args": []
    },
    "stata-mcp": {
      "command": "uvx",
...
5.  **实操设计题**：请设计一个MCP组合编排工作流，实现「自动监控某上市银行官网公告，当出现新的利润分配公告时，自动提取关键数据，生成分析简报（Word+PPT），并发送通知」。要求：
...
:::
   ...
   **\#**  **交付物**                                         **形式**
  -------- -------------------------------------------------- ----------------------------------
     1     4关每关一段录屏GIF（≤`<!-- -->`{=html}10s）   gif/exp2_L1.gif \... exp2_L4.gif
