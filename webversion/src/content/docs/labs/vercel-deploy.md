---
title: 'CNB流水线自动部署到Vercel'
description: '使用 CNB 云原生构建流水线，实现代码推送后自动部署静态网站到 Vercel'
---

实验步骤：CNB 云原生构建 + Vercel 自动部署

本实验指导学生配置 CNB（Cloud Native Build）流水线，实现"代码推送即部署"的持续集成/持续部署（CI/CD）工作流，将课程网站自动发布到 Vercel。
...
CI/CD（持续集成/持续部署）是现代软件开发的核心实践：
  ...
  CD（持续部署）    构建通过后自动发布到生产环境            构建完成后自动部署到 Vercel
...
核心组件：
  ...
  Vercel                全球 CDN 静态网站托管               webversion-alpha.vercel.app
...
main:                              # 目标分支：main
  push:                            # 触发事件：代码推送到 main 分支时
    - name: deploy-webversion-to-vercel
      ...
      imports:
        # 从密钥仓库导入 Vercel Token（敏感信息不存放在代码中）
...
含义：从 CNB 的密钥仓库中导入敏感信息（Vercel Token）。密钥仓库是一种特殊类型的仓库，专门存放密码、API Key 等敏感数据，禁止 Git Clone 到本地，只能通过流水线引用。

⚠️ 安全要点：Token 等敏感信息绝不能写入代码文件，必须使用密钥仓库管理。
...
流水线包含 4 个顺序执行的 stages（阶段）：
  ...
  部署 Vercel vercel deploy --prod --token=... --yes            上传到 Vercel CDN
...
(3) 填写信息：
  ...
  Name        cnb-deploy
  Scope       选择你的团队
  Expiration  1 Year（或自定义更长时间）

(4) 点击「Create」，立即复制生成的 Token（形如 vercel_xxxx）
...
三、添加 Vercel Token 到密钥仓库

(1) 进入密钥仓库页面，点击「在线编辑」
(2) 新建文件 `vercel.yml`，内容为：

```yaml
VERCEL_TOKEN: "粘贴你的Vercel API Token"
```
...
密钥仓库配置完成后，推送任意提交即可触发构建：
  ...
  git commit --allow-empty -m "trigger: 首次触发CNB自动部署"
...
方式 B：网页查看
  ...
  打开 https://cnb.cool/你的用户名/smartbanking/-/build
  可以看到所有构建记录和详细日志
...
构建成功后（通常 1-2 分钟），打开网站确认更新：
  ...
  访问 https://webversion-alpha.vercel.app（或你自己的 Vercel 域名）
  确认页面显示最新内容
...
========================================
  ...
  imports 失败 / Secret repos do not support  密钥仓库路径错误或权限不足        检查 .cnb.yml 中 imports URL 是否正确
  Token 无效 / Authentication error           Vercel Token 过期或错误           重新创建 Vercel Token，更新密钥仓库
  Build failed (astro build 报错)             Markdown 语法错误或依赖缺失       先在本地 pnpm build 验证通过再推送
  Deploy timeout                              Vercel 服务异常或网络超时        等待几分钟后重新推送触发
  node_modules 相关错误                        依赖版本冲突                     删除 pnpm-lock.yaml 后在本地重新 pnpm install
...
二、本地预览 vs 线上部署
  ...
  手动部署      cd webversion && npx vercel --prod https://webversion-alpha.vercel.app
  自动部署      git push origin main              同上（CNB 流水线自动执行）

⚠️ 建议：推送前先用 pnpm dev 在本地预览确认效果，避免推送后发现问题。
