...
MiKTeX 首次编译会自动下载缺失宏包（如 ctex 系列），弹出提示点击 Install 即可。首次耗时约 1--2 分钟，后续约 5--10 秒。macOS 使用 MacTeX 则无需额外下载，已包含全部宏包。详细配置请参见\textbf{附录 E}。
\end{notebox}
...
Stata（实验二 L4 关） & 如未安装 Stata，可参考附录 C，或在实验报告中注明用 Python statsmodels 替代 \\
\bottomrule
\end{tabular}
\end{table}
...
\section{附录D CNB 与 GitHub 命令对比速查表}
